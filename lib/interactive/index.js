...
class InteractiveUI {
  ...
  _ensureInterface() {
    if (!this._rl) {
      ...
      this._cleanup = handleSigInt(this._rl);
    }
  }
  ...
  async promptForFileConflict(filePath, force = false) {
    if (force) {
      return 'overwrite';
    }
  ...
  async confirm(message, defaultValue = 'N') {
    this._ensureInterface();
    const rl = this._rl;

    try {
      while (true) {
        const answer = await question(rl, message, defaultValue);
        const lowerAnswer = answer.toLowerCase();

        if (lowerAnswer === 'y' || lowerAnswer === 'yes') {
          return true;
        } else if (lowerAnswer === 'n' || lowerAnswer === 'no' || lowerAnswer === '') {
          return false;
        } else {
          console.log('  Please enter y or n.');
        }
...
module.exports = {
  ...
  interactiveUI,
  question,
  promptForVariables: (...args) => interactiveUI.promptForVariables(...args),
  promptForFileConflict: (...args) => interactiveUI.promptForFileConflict(...args)
};
